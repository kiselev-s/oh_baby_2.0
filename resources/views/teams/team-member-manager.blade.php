<div>
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-form-section submit="addTeamMember">
                ...
                <x-slot name="form">
                    ...
                    @if (count($this->roles) > 0)
                        <div class="col-span-6 lg:col-span-4">
                            ...
                            <div class="relative z-0 mt-1 border border-gray-200 dark:border-cyan-700 rounded-lg cursor-pointer">
                                @foreach ($this->roles as $index => $role)
                                    <button type="button" class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 dark:focus:ring-cyan-700 {{ $index > 0 ? 'border-t border-gray-200 dark:border-cyan-700 rounded-t-none' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
                ...
                <x-slot name="actions">
                    <x-jet-action-message class="mr-3 dark:text-gray-400" on="saved">
                        {{ __('Added.') }}
                        ...
                        @foreach ($team->users->sortBy('name') as $user)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4 dark:text-gray-400">{{ $user->name }}</div>
                                </div>
    ...
    <x-jet-dialog-modal wire:model="currentlyManagingRole">
        ...
        <x-slot name="content">
            <div class="relative z-0 mt-1 border border-gray-200 dark:border-cyan-700 rounded-lg cursor-pointer">
                @foreach ($this->roles as $index => $role)
                    <button type="button" class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 dark:focus:border-cyan-700 focus:ring focus:ring-blue-200 dark:focus:ring-cyan-700 {{ $index > 0 ? 'border-t border-gray-200 dark:border-cyan-700 rounded-t-none' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
